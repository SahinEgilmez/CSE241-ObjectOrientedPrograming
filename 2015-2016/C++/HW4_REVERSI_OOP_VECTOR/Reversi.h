...
class Cell {//cell class represent the Points coordinates for Reversi game
public:
    Cell(); //constructor
    ...
    inline int getX(); //get line number(x)
    inline int getY(); //get column number(y)
    inline void setX(int number); //set line number(x)
    inline void setY(int number); //set column number(y)
private:
    char owner; //take cell owner
    int cellX; //take line number on the board
    int cellY; //take column number on the board
    int livingCell; //take owner with integer.1=computer,2=user
};

class PlayableCells {//this class represent the playable Cells at the moment
public:
    PlayableCells(); //constructor
    int x; //line value on the board
    int y; //column value on the board
    int eatablePoints; //if player play this Cell,eat cells until eatablePoints
    int eatDirections; //take the eat cell direction.Directions are 0,1,2,3,4,5,6,7
    //with clockwise
};
...
inline int Cell::getX() {
    return cellX;
}

inline int Cell::getY() {
    return cellY;
}
...
void Reversi::writeFile(int player, int x, int y) {
    ...
    if (player == 2) {
        file << endl << moveNumber << " U ";
        moveNumber++;
    } else if (player == 1) {
        file << endl << moveNumber << " C ";
        moveNumber++;
    }
    file << x << "*" << y << " ";
    file.close();
