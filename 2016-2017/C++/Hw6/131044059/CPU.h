...
public:
    ...
    bool execute(string line, Memory &memory); //main function
...
private:
    ...
    bool arithmetics(const string regFirst, const string regSecond, int optReg[4]);
    /*this function execute mov instruction.optReg which decides to use register */
    void mov(const int optReg[3], Memory &memory);
    /*this function execute add instruction.optReg which decides to use register */
    void add(const int optReg[3], Memory &memory);
    /*this function execute sub instruction.optReg which decides to use register */
    void sub(const int optReg[3], Memory &memory);
    /*this function print to register or any integer value*/
    void prn(const string reg, Memory &memory);
    /*jmp  go to the line.*/
    void jmp(const string instructions[3], const int optReg[4]);
    /*jpn  go to the line.*/
    void jpn(const string instructions[3], const int optReg[4]);
