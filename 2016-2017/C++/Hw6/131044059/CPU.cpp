...
bool CPU::execute(string line, Memory &memory) {
    ...
    if (getOption() == 1) {//display each instruction
        ...
        if (instructions[1].substr(0, 1) == "#")
            ...
            "=" << (int) memory.getMem(atoi(instructions[1].substr(1,
                instructions[1].size() - 1).c_str()) - 1) << ", ";
        if (instructions[2].substr(0, 1) == "#")
            cout << instructions[2].substr(0, instructions[2].size()) <<
            "=" << (int) memory.getMem(atoi(instructions[2].substr(1,
                instructions[2].size() - 1).c_str()) - 1) << ", ";
        cout << endl;
    }
...
bool CPU::checkError(string line) {
    ...
    if (line[i] == 'R' && !((atoi(line.substr(i + 1, 1).c_str()) > 0) &&
            (atoi(line.substr(i + 1, 1).c_str()) < 6))) {
        return true;
    }
    if (line[i] == '#' && !((atoi(line.substr(i + 1, 1).c_str()) >= 0) &&
            (atoi(line.substr(i + 1, 1).c_str()) < 50))) {
        return true;
    }
...
int CPU::parser(string line, string instructions[3]) {
    ...
    if (instructions[0] == "MOV" || instructions[0] == "ADD" || instructions[0] == "SUB") {
        if (instructions[0] == "MOV" && line.substr(3, 1) == "#") {
            ...
            if (atoi(instructions[1].substr(1, instructions[1].size() - 1).c_str()) > 49
                    || atoi(instructions[1].substr(1, instructions[1].size() - 1).c_str()) < 0)
                return -5;
...
bool CPU::arithmetics(const string regFirst, const string regSecond, int optReg[4]) {
    ...
    if (regFirst.substr(0, 1) == "#") {
        optReg[0] = atoi(regFirst.substr(1, regFirst.size() - 1).c_str());
    ...
    if (regSecond.substr(0, 1) == "#") {
        optReg[1] = atoi(regSecond.substr(1, regSecond.size() - 1).c_str());
...
void CPU::jmp(const string instructions[], const int optReg[]) {
    if (instructions[2] == "." || (getRegWithPos(optReg[0]) == 0)) {
        if (instructions[2] == ".")//Example : JMP 5
            setPC(atoi(instructions[1].c_str()) - 1);
        else //Example : JMP R1, 5
            setPC(atoi(instructions[2].c_str()) - 1);
    }
}
...
void CPU::prn(string reg, Memory &memory) {
    ...
    if (reg.substr(0, 1) == "#") {
        cout << ((int) memory.getMem(atoi(reg.substr(1, reg.size() - 1).c_str()))) << endl;
    } else if (reg == "R1")
        cout << getR1() << "\n";
    else if (reg == "R2")
        cout << getR2() << "\n";
