...
bool CPU::checkError(string line) {
    ...
    str = line.substr(i, 3);
    if (!(str == "MOV" || str == "ADD" || str == "SUB" || str == "JMP" || str == "JPN" ||
            str == "PRN" || str == "HLT"))
        return true;
    i = i + 3;
    while (i < line.size()) {
...
void CPU::mov(const int optReg[], Memory &memory) {
    ...
    if (optReg[2] == 0 && optReg[3] == 0)
        ...
        //setRegWithPos(optReg[0], memory.getMem(optReg[1])); //register = adress
        ...
        //memory.setMem(optReg[0], getRegWithPos(optReg[1])); // adress = register
